...
public class HashTwoSumTest {

	private TwoSum twoSum;
	private TwoSum dupes;
	private TwoSum randomLarge;
	private TwoSum randomLarger;
	@Before
	public void setUp() throws Exception {

		twoSum=new HashTwoSum();
		twoSum.store(1);
		twoSum.store(5);
		twoSum.store(6);
		dupes=new HashTwoSum();
		dupes.store(3);
		dupes.store(3);
		dupes.store(9);
		dupes.store(-100);
		...
		randomLarger=new HashTwoSum();
		long then=System.currentTimeMillis();
		for(int i=0;i<=10000;i++) {
			Random gen=new Random();
			int val=gen.nextInt();
			randomLarger.store(val);
		}
	...
	public void test() {
		...
		for(int i=0;i<20000;i++) {
			randomLarger.isSum(gen.nextInt());
		}
		System.out.println(System.currentTimeMillis()-then);
		then=System.currentTimeMillis();
